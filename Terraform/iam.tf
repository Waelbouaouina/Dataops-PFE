...
//
// IAM bindings pour le Service Account dataloader-sa
//
...
resource "google_project_iam_member" "dataloader_sa_bigquery_data_editor" {
  project = var.project_id
  role    = "roles/bigquery.dataEditor"
  member  = "serviceAccount:${google_service_account.dataloader_sa.email}"
}

resource "google_project_iam_member" "dataloader_sa_storage_object_viewer" {
  project = var.project_id
...
//
// IAM bindings pour ton compte Terraform (var.terraform_sa_email)
//, afin d’autoriser la création des ressources
//
...
//
// Grant Composer Service Agent Extension role to Composer service agent
//
